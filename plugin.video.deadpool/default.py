...
else: SOURCES = [{"url": "http://bit.ly/deadpooldona", "fanart": "http://bit.ly/dptapiz"}]
	...
	try:
		if os.path.exists(favorites) == True:
			...
			if FAV == "[]":
				...
				addDir('[COLOR red][B]- MIS CANALES FAVORITOS[/COLOR][/B]','url',4,os.path.join(home, 'resources', 'favorite.png'),FANART,'','','','')
				addDir('','',100,'',FANART,'','','','')
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(deadpool,New source added.,5000,"+icon+")")
        if not url is None:
